...
const OrderCategories: FC<Props> = ({data, orderId, updateHandler}) => {
  ...
  const changeHandler = (isChecked: boolean, selectedData: any, id: string, key: string): void => {
    if (isChecked) {
      const temp = {
        groupId: id,
        orderId: orderId,
          ...
          return (
            ...
            <Box key={"ORDER_CATEGORY_ITEM_" + index} display="flex" flexDirection="column" gap="18px">
                ...
                <OrderProducts data={value}
                               ...
                               handleChange={(isChecked, selectedData, groupId) => changeHandler(isChecked, selectedData, groupId, key)}/>
            </Box>
          )
        })
      }
