...
interface Props {
  selectedItems: { orderId:string,groupId: string, currentStatus: string, items: Array<any> } | null,
  closeHandler: () => void
  updateHandler: () => void
}
  ...
  const submitHandler = () => {
    request({
      ...
      data:{
        ...
        newStatus:newStatus,
        subProducts:finalValues.items.map(item=>({id:item._id,quantity:item.quantity}))
      }
    }).then(res=> {
          ...
          {finalValues?.items?.map((item, index) => (
            <Box className="change-status-products" key={"PRODUCT_ITEM_" + index}>
              <Box display="flex" gap="8px" alignItems="center">
                <Image className="product-image-container" src={item.thumbnail} alt={item.title} width={36} height={36}
                       layout="fixed"/>
